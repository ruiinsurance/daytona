...
function setup() {
  ...
  const generationRepository = {
    findOne: vi.fn(async ({ where }: any) => generationRows.find((row) =>
      row.placementId === where.placementId && row.generation === where.generation) ?? null),
...
function checkpoint() {
  const objects = [{ key: 'state.db', body: 'state', size: 5, sha256: 'file-hash' }]
  const manifest = {
...
describe('WorkspaceGenerationService', () => {
  ...
  it('uploads immutable files, manifest, read-back, commit marker, then latest in order', async () => {
    ...
    const store = {
      ...
      readManifest: vi.fn(async () => { events.push('readback'); return snapshot.manifest }),
      putCommittedMarker: vi.fn(async () => events.push('committed')),
      getLatest: vi.fn(async () => { events.push('get-latest'); return null }),
      compareAndSetLatest: vi.fn(async () => { events.push('latest'); return true }),
    }
    const source = { create: vi.fn(async () => snapshot) }

    const result = await service.reconcile({ placementId: PLACEMENT_ID, source, store, now: new Date('2026-08-17T00:00:00.000Z') })
    ...
    expect(source.create).toHaveBeenCalledWith(expect.objectContaining({
      ...
      fenceEpoch: '1',
      leaseOwner: `generation-worker:${PLACEMENT_ID}`,
    ...
    expect(workspacePlacementService.releaseWriterLease).toHaveBeenCalledWith(expect.objectContaining({
      placementId: PLACEMENT_ID,
      fenceEpoch: 1,
      leaseOwner: `generation-worker:${PLACEMENT_ID}`,
    }))
  ...
  it('retains failed generations and never advances latest after partial upload', async () => {
    ...
    const store = {
      putObjects: vi.fn(async () => events.push('objects')),
      putManifest: vi.fn(async () => { events.push('manifest'); throw new Error('COS body omitted from assertion') }),
      readManifest: vi.fn(),
    ...
    expect(generationRows[0]).toMatchObject({ state: WorkspaceGenerationState.FAILED, errorCode: 'generation_upload_failed' })
