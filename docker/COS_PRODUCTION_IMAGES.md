...
```text
源提交：620567fe1cb98a23d7aa143c9d9ca4821ab6dabd
版本：  v0.190.0-cos-620567fe
平台：  linux/amd64
...
./scripts/cos-production-images.sh plan \
  ...
  --output-dir /private/tmp/daytona-cos-production-images/v0.190.0-cos-620567fe
```

计划应包含以下本地引用：

```text
daytona-local/daytona-api:v0.190.0-cos-620567fe
daytona-local/daytona-runner-base:v0.190.0-cos-620567fe
daytona-local/daytona-proxy:v0.190.0-cos-620567fe
daytona-local/daytona-ssh-gateway:v0.190.0-cos-620567fe
```
...
org.opencontainers.image.revision=620567fe1cb98a23d7aa143c9d9ca4821ab6dabd
org.opencontainers.image.version=v0.190.0-cos-620567fe
```
...
```text
api_v0.190.0-cos-620567fe_linux-amd64.tar
runner_v0.190.0-cos-620567fe_linux-amd64.tar
proxy_v0.190.0-cos-620567fe_linux-amd64.tar
ssh-gateway_v0.190.0-cos-620567fe_linux-amd64.tar
IMAGE-MANIFEST.tsv
...
脚本先通过同级锁目录串行化同一输出目标，再在输出目录同级创建临时 staging。四个 tar、两个 manifest 及 SHA-256 回验全部完成后，才通过一次目录改名发布最终输出。任一阶段失败都会清理 staging 和本进程持有的锁，不留下看似完整的部分交付目录；若发现同目标锁已存在，脚本会立即拒绝并要求操作者先确认是否仍有导出进程，不能擅自删除锁。

Linux 回验：

```bash
cd /private/tmp/daytona-cos-production-images/v0.190.0-cos-620567fe
...
```bash
shasum -a 256 api_v0.190.0-cos-620567fe_linux-amd64.tar
# 传输并在目标端核验后：
rm -- api_v0.190.0-cos-620567fe_linux-amd64.tar
```
